...
const folds = lines.filter(l => l.startsWith('fold along ')).map(l => l.split('=')).map(([k, line]) => [k.slice(-1), +line]);
...
const doFold = {
  x: (matrix, fold) => Object.keys(matrix)
    .reduce((obj, k) => ({
      ...obj,
      [k]: new Set(
        [...matrix[k]].map(v => v < fold ? v : 2 * fold - v)
      )
    }), {}),
  y: (matrix, fold) => Object.keys(matrix)
    .map(Number)
    .reduce((obj, k) => {
