...
const getDigitMap = patterns => {
  ...
  const subset = (a, containsB) =>
    intersect(a, containsB).length === containsB.length;
  ...
  // 3 contains 1
  digitMap[3] = patternsByLength[5].filter(pattern =>
    subset(pattern, digitMap[1])
  )[0];
  // 5 unioned with 1 contains 4
  digitMap[5] = patternsByLength[5].filter(pattern =>
    subset(union(pattern, digitMap[1]), digitMap[4])
  )[0];
  // 2 is the only 5-length left
  digitMap[2] = diff(patternsByLength[5], [digitMap[3], digitMap[5]])[0];

  // 9 contains 4
  digitMap[9] = patternsByLength[6].filter(pattern =>
    subset(pattern, digitMap[4])
  )[0];
  // 0 contains 1, and isn't nine
  digitMap[0] = patternsByLength[6].filter(
    pattern => subset(pattern, digitMap[1]) && pattern !== digitMap[9]
  )[0];
  // 6 is what's left
  digitMap[6] = diff(patternsByLength[6], [digitMap[9], digitMap[0]])[0];

  return Object.values(digitMap).reduce(
    (map, digit, i) => ({ ...map, [digit.join('')]: i }),
    {}
  );
};
