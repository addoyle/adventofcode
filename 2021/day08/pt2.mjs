...
const getDigitMap = patterns => {
  ...
  const diff = (elementsIn, butNotIn) => elementsIn.filter(i => !butNotIn.includes(i));
  ...
  const patternsByLength = patterns
    ...
    .reduce(
      (pbl, pattern) => ({
        ...pbl,
        [pattern.length]: pbl[pattern.length] ? [...pbl[pattern.length], pattern] : [pattern]
      }),
      {}
    );
